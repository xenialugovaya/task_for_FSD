  ...
  $.fn.iqDropdown = function (options) {
    this.each(function () {
      ...
      function updateDisplay () {
        ...
        const useDefault = totalItems == 0 && settings.textDefault.length > 0;
        const usePlural = totalItems > 1 && settings.textPlural.length > 0;
        const text = usePlural ? settings.textPlural : settings.selectionText;
        const writeText = useDefault ? settings.textDefault : text;
        useDefault ? $selection.html(`${writeText}`) : $selection.html(`${totalItems} ${writeText}`);
      }
